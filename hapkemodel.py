...
def ssd(func, guess, obs, ph):
    ...
    sol, cov, infodict, mesg, ier = opt.leastsq(
        res, guess, args=(obs, ph), full_output=True
    )
...
class HapkeModel:
  ...
  """
    ...
    global deque, pi, vec, array, linspace, arange, P, quad, simps, log, sqrt, sum, exp, sin, cos, tan, indices
    ...
    def __init__(self, phase_range, **args):
        ...
        if isinstance(phase_range, list) == False:
            ...
            if phase_range[1] > 5e0:
                g = radians(
                    list(arange(phase_range[0], 5e0, 0.25 * phase_range[2]))
                    + list(arange(5e0, phase_range[1], phase_range[2]))
                )
            else:
        ...
        if "X" in args:
            Qsca, Qe, Qback, gsca = self.Sph_Sca_mie(
                self.args["X"], self.args["refr"], 50
            )
    ...
    def M(self, m, m0, b):
        ...
        H_o2 = vec(
            ...
            / (
                ...
                - w
                * x
                * (r0 + 0.5 * (1e0 - 2e0 * r0 * x) * log((1e0 + x) / (x + TINY)))
            )
        )
    ...
    def Bsh(self, Bs0, **args):
        ...
        if "v" in args and "fill_factor" in args:
            ...
            if v == None:
                N = sqrt(3e0 / 8)  # K*a*exp(-a/a_mean)

            # K*a**(-n)
            if v == 0:
                N = 4 / (3 * sqrt(3e0))  # n = 0
            if v == 1:
                N = 3e0 / (sqrt(8e0 * log(a_ratio)))  # n = 1
            if v == 2:
                N = 2e0 * sqrt(1e0 / a_ratio)  # n = 2
            if v == 3:
                N = sqrt(2e0) * (log(a_ratio) ** (3e0 / 2)) * (1e0 / a_ratio)  # n = 3
            if v == 4:
                N = sqrt(3e0) / log(a_ratio)  # n = 4
            if v == 5:
                N = 1e0 / sqrt(2e0)  # n = 5
    ...
    def Aw(self, g, Bs0, Bc0, **args):
        ...
        for phase in g:
            ...
            iMsca.append(
                simps(simps((m0 * m / (m0 + m)) * self.M(m, m0, b) * dA, axis=1))
            )
